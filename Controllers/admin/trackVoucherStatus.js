  ...
  try {
    const response = await Vouchers.findOne({
      ...
      include: [
        {
          model: userTable,
          attributes: ["firstName", "lastName", "designation"],
        },
    ...
    if (!response) {
      return res.status(200).json({response: [], msg: "no data found for that voucher" });
    }
    ...
    return res
      .status(400)
      .json({ msg: "Error while finding the data", err: err });
  }
};

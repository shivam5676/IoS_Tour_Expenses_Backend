...
module.exports = async (req, res) => {
  ...
  if (statusFrom == statusTo) {
    return res.status(400).json({
      msg: `You can not change voucher status - ${statusFrom} to ${statusTo}....because both are same `,
    });
  }
  ...
  if (
    ...
    return res.status(400).json({
      msg: `Pending Voucher Can not be processed to accepted or rejected`,
    });
  }
  if (!userId) {
  ...
  try {
    if (statusFrom == "Accepted") {
      ...
      if (!updatedData) {
        return res
          .status(400)
          .json({ msg: "only accepted voucher can be change" });
      }
      ...
      const assignedVoucherResponse = await assignedVoucher.findOne({
        where: {
          assignedTo: userId,
          status: "Accepted",
          voucherId: voucherId,
    ...
    if (statusFrom == "Rejected") {
      ...
      if (!updatedData) {
        return res
          .status(400)
          .json({ msg: "only Rejected voucher status can be change" });
      }
      await updatedData.update({
        statusType: statusTo,
